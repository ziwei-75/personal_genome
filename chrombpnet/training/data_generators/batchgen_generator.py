...
class ChromBPNetVariantGenerator(keras.utils.Sequence):
    """
    This generator randomly crops (=jitter) and revcomps training examples for 
    every epoch, and calls bias model on it, whose outputs (bias profile logits 
    and bias logcounts) are fed as input to the chrombpnet model.
    """
    def __init__(self, peak_regions, nonpeak_regions, genome_fasta, batch_size, inputlen, outputlen, max_jitter, negative_sampling_ratio, add_revcomp, return_coords, shuffle_at_epoch_start,peak_region_keep_idx, nonpeak_region_keep_idx, sample_list,bigwig_list):
        """
        seqs: B x L' x 4
        cts: B x M'
        inputlen: int (L <= L'), L' is greater to allow for cropping (= jittering)
        ...
        batch_size: int (B)
        """
        print("load VCF to train from personal genome...")
        self.bigwig_list = bigwig_list
        peak_seqs, peak_cts, peak_coords, nonpeak_seqs, nonpeak_cts, nonpeak_coords, = data_utils.load_data_with_variants(peak_regions, nonpeak_regions, genome_fasta, inputlen, outputlen, max_jitter,sample_list,bigwig_list,peak_region_keep_idx,nonpeak_region_keep_idx)
        self.peak_seqs, self.nonpeak_seqs = peak_seqs, nonpeak_seqs
        self.peak_cts, self.nonpeak_cts = peak_cts, nonpeak_cts
        self.peak_coords, self.nonpeak_coords = peak_coords, nonpeak_coords
    ...
    def crop_revcomp_data(self):
        ...
        self.cur_seqs, self.cur_cts, self.cur_coords = augment.crop_revcomp_augment(
                                            self.seqs, self.cts, self.coords, len(self.bigwig_list), self.inputlen, self.outputlen, 
                                            self.add_revcomp, shuffle=self.shuffle_at_epoch_start
                                          )
